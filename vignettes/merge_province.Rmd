...
The package `poseid` contains different tools. The function `merge_prov` demonstrate in this vignette provides a method that efficiently generate time series with common spatial definitions over a given time range.
...
As example, we use the data frame containing the information on the `birth and death rate`:
...
To have a better view of how the function works, we will select only view provinces concerned by some events of spliting or merging.
...
Automaticaly, the time range selected is `from` to `to = 2017-12-31`. But we can change the time-range, we can use the parameters `from` and `to`, the only restriction is that the time-range selected overlaps the time range of the data frame inputted. 
...
By default, the province are merge back together by applying the function `sum`. We can change this by using the function `FUN`. 
...
We can also use weighted mean by providing the weighted in another data frame, for example by the size of the population. 
For that, we can use the data frame `pop_size` provides in the `gso` package. The data been expressed in thousand people, we will change it.
...
We can now used the pop_size information in the function with the parameters `df2` and `args` to calculate the weighted mean by the total size of the population.
...
The parameters `df` and `df2` should have at least one column in commun to be able to join them together. 
...
It can be interesting to link the epidemiological data from the `gdpm` package with the informations from the `gso` package.  
...
So, if we want to link the data frame to be express with the same province as the `hepatitis` or `amoebiasis` data frame, we can use the parameter `diseases`. 
